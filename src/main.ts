...
class AutoStrom extends utils.Adapter {
    ...
    private async checkForNewPower(): Promise<void> {
        ...
        this.log.debug(
            '------------ ' +
                total_available +
                'W (' +
                (total_available > 0 ? 'unter null' : 'über null') +
                ') ------------',
        );
    ...
    private async startIteratingDevices(devices: AutoStromDevice[], total_available: number): Promise<number> {
        ...
        this.log.debug(
            '-> FERTIG, geändert: ' +
                changedPower +
                'W, warten für mind. ' +
                Math.round(next_call_time / 100) / 10 +
                's.',
        );
            ...
            if (result === 0) {
                if (j === analog_devices.length - 1) {
                    if (changed === 0 && mode_under_null) {
                        this.log.info(
                            '!! Es wird eingespeist und es kann nicht mehr geregelt werden, da alle Verbraucher bereits voll an und alle Erzeuger aus sind',
                        );
                    } else {
    ...
    private changeDigital(
        device: DigitalDevice,
        available_power: number,
        mode_under_null: boolean,
        analog_devices: AnalogDevice[],
    ): number | boolean {
        ...
        if (device.delay_ms > 0 && this.delay_dict[device.id] !== undefined && this.delay_dict[device.id] > now) {
            if (device.erzeuger && !mode_under_null) {
                ...
                this.log.debug(
                    log_name +
                        ' wird nicht geschaltet, da es noch Schaltverzögerung von ' +
                        (this.delay_dict[device.id] - now) / 1000 +
                        's hat, continue',
                );
        ...
        if (mode_under_null)
            this.log.info('Zu viel Erzeugung da, ' + log_name + ' wird geregelt (Verbraucher an, Erzeuger aus)');
    ...
    private changeAnalog(analog_device: AnalogDevice, available_power: number): number {
        ...
        if (isNaN(new_percentage)) {
            this.log.error(
                'Sollte niemals passieren: Analoger Verbraucher kann nicht auf ' +
                    new_percentage +
    ...
    private shouldAnalogBeUsed(
        device: AutoStromDevice,
        available_power: number,
        mode_under_null: boolean,
        log_name: string,
        analog_devices: AnalogDevice[],
        ...
        if (mode_under_null && consumable_from_analog > available_power) {
            if (this.WINTER_MODUS || device.erzeuger) {
                ...
                this.log.debug(
                    'Die zu viele Erzeugung kann durch Nachregeln von analog ausgeglichen werden, break (winter mode oder erzeuger)',
                );
                return true;
            }
        }
        ...
        if (device.verbrauch > max_compensable) {
            if (!mode_under_null && freeable_from_analog === 0) {
                ...
                this.log.debug(
                    'Über der Nulllinie und nichts mehr frei in analog: Digitaler wird geschaltet, um unter Nulllinie zu bleiben.',
                );
            } else {
                this.log.debug(
                    log_name +
                        ' nicht geändert, da danach nicht mit analogem kompensiert werden kann. Verbrauch ' +
                        device.verbrauch +
                        'W und max_compensable ist ' +
                        Math.round(max_compensable) +
                        'W => Regeln nur mit analog oder gar nicht.',
                );
                return true;
            }
        }
